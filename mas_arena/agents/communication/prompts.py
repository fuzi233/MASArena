...
class PromptComposer:
    ...
    @staticmethod
    def json_repair_prompt(*, invalid_output: str, action_kind: Literal["solver", "aggregator", "legacy_aggregator"]) -> list[dict[str, str]]:
        if action_kind == "legacy_aggregator":
            ...
            examples = 'Choose exactly one: {"action":"think","reasoning_note":"brief review progress"}; {"action":"ask","reasoning_note":"specific uncertainty","recipient_id":"<eligible_solver_id>","question":"focused question"}; {"action":"submit","reasoning_note":"final concise rationale","final_answer":"exact answer"}.'
    ...
    def legacy_final_aggregator_prompt(*, problem: str, solver_reports: list[dict[str, object]], state: CommunicationState) -> list[dict[str, str]]:
        return [{"role": "system", "content": "You are the final aggregator. First reason internally over the reports and review events. Either select one solver candidate, or rederive the answer from the trajectory. Return exactly two adjacent XML elements and no other text. First: <aggregation_decision>{\"mode\":\"select_candidate\" or \"rederive_from_trace\",\"source_solver_ids\":[\"solver_1\"],\"selected_candidate_solver_id\":\"solver_1\" or null,\"rationale\":\"brief evidence-based reason\"}</aggregation_decision>. For select_candidate, selected_candidate_solver_id is required. For rederive_from_trace, source_solver_ids must name every solver report or message trajectory materially used. Second: <final_answer>the exact answer required by the task</final_answer>. Inside final_answer, include only the answer value in the exact form requested by the problem. Do not include reasoning, Markdown, labels, explanations, or restate the question. For a binary question, output exactly `Yes` or `No`. For a number, output only the number; never add words such as `Position` or `Answer`. For multiple-choice or list answers, preserve the exact separators and ordering requested by the problem."}, {"role": "user", "content": json.dumps({"problem": problem, "solver_reports": solver_reports, "review_events": state.public_transcript("aggregator")}, ensure_ascii=False)}]
